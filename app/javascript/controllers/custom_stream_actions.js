...
StreamActions.update_task_unless_up_to_date = function() {
  ...
  const position = Number(fragment.firstElementChild.querySelector("input[name='task[position]']").value)

  if (document.getElementById("version")?.innerText !== version) {
    document.getElementById(taskId).remove()
    ul.insertBefore(fragment.firstElementChild, ul.children[position].nextSibling)
  }
}
